...
function install (args, cb) {
  ...
  var root = path.resolve(npm.dir, "..")

  // `npm install`
  // read package.json of current path and install
  if (args.length === 0) {
    return real_install(null, root, {}, cb);
  }

  // things to install specified on command line
  var where = path.resolve(root, 'node_modules');

  // using this approach, we should keep track of
  (function next(err) {
    if (err) {
      return cb(err);
    }
    ...
    unpack(what, where, {}, next);
  })();
}

// need to install a given 'what' into 'where'
// what is an acceptable installation string
// root is a node_modules path
function unpack(what, root, installed, cb) {

  // need way to ask cache if 'what' already exists

  // query the cache for details about 'what'
  console.log('fetching', what);
  cache.add(what, function (err, data) {
    if (err) {
      return cb(err);
    }

    console.log('fetched %s@%s', data.name, data.version);
    //chain[dep] = data.version;

    var where = path.resolve(root, data.name);

    var up = npm.config.get("unsafe-perm");
    var user = up ? null : npm.config.get("user");
    var group = up ? null : npm.config.get("group");
    cache.unpack(data.name, data.version, where, null, null, user, group, function(err) {
      if (err) {
        return cb(err);
      }

      console.log('unpacked', where);

      // unpacked, do the rest of the install as if this was root
      real_install(data, where, {}, cb);
    });
  });
}

function real_install (info, root, installed, cb) {
  if (info) {
    console.log('install', info.name, info.version, root);
  }
  else {
    console.log('install', root);
  }

    // this will be the dependencies available to us
    var chain = xtend({}, installed);

    // where modules will be installed for this package
    var where = path.resolve(root, 'node_modules');

    var opt = {};

    readDependencies(null, root, opt, function (err, data) {
      if (err) {
        return cb(err);
      }
      ...
      // list of dependencies we will need to install
      var children = [];

      var names = Object.keys(deps);
      var done = after(names.length, install_deps);

      names.forEach(function(dep) {
        var details = deps[dep];
        var info = {
            name: dep,
            version: details
        };

        // already have this installed higher up
        var prev = installed[dep];
        if (semver.satisfies(prev, details)) {
            return done();
        }
        ...
        var what = details;
        var parsed = url.parse(details);

        // if details is not a url, module@version
        if (!parsed.protocol) {
            what = dep + '@' + details;
        }

        // TODO(shtylman) read package.json for this dependency if available
        // if version matches what we expect, then don't add to cache
        // what about when we don't know what version we expect?
        // can the cache tell us what version for a 'what' string we should have?
        //
        // we should be able to ask the cache
        // for the version of a given 'what'
        // the cache can then tell us if we need to fetch or not
        //
        // simple approach, just check path for package.json

        var where = path.resolve(root, 'node_modules', dep);

        // TODO(shtylman)
        // given a 'what' the cache should be able to give us the 'data'
        // this should be the same 'data' as doing an 'add' provides

        var pkg_path = path.resolve(where, 'package.json');

        if (fs.existsSync(pkg_path)) {
          var pkg_info = JSON.parse(fs.readFileSync(pkg_path, 'utf8'));

          // TODO(shtylman) if details is not a version-ish
          // then this is useless
          // we need the cache to tell us wtf
          if (semver.satisfies(pkg_info.version, details)) {
            chain[dep] = pkg_info.version;
            return done();
          }
        };
        ...
        cache.add(what, function (err, data) {
          ...
          console.log('fetched %s@%s', dep, data.version);
          chain[dep] = data.version;

          var up = npm.config.get("unsafe-perm");
          var user = up ? null : npm.config.get("user");
          var group = up ? null : npm.config.get("group");
          cache.unpack(dep, data.version, where, null, null, user, group, function(err) {
            if (err) {
              return cb(err);
            }
      ...
      // recurse install each dep now
      function install_deps(err) {
        if (err) {
          return cb(err);
        }

        var dep = children.shift();
        if (!dep) {
          return cb();
        }

        var dep_root = path.resolve(where, dep.name);

        // now we can install all the deps below this level
        // since we know what
        real_install(dep, dep_root, chain, function(err) {

          // once the deps are installed
          // we can proceed to next step (build or whatever)
          install_deps(err);
        });
      }
    });
...
function install_orig (args, cb_) {
  ...
  // if specified on command line, just install that into where
  // otherwise
  // read dependencies for where we are
  // figure out what version to install of dependency
  // fetch dependency
  // install dependency (runs the install on a new location)
