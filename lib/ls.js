...
function ls (args, silent, cb) {
  ...
  function trim_deps(data, depth) {
    var deps = data.dependencies;
    if (depth === 0) {
      for (var name in deps) {
        deps[name].dependencies = undefined;
      }
      return;
    }

    for (var name in deps) {
      trim_deps(deps[name], depth - 1);
    }
  }

  var depth = npm.config.get("depth")
  readInstalled(dir, null, log.warn, function (er, data) {
    var bfs = bfsify(data, args)
      , lite = getLite(bfs)

    if (depth !== null) {
      trim_deps(data, depth);
    }
    ...
    if (deps.length) lite.dependencies = deps.map(function (d) {
      ...
      if (typeof dep === "string") {
        ...
        if (data.depth >= maxDepth) {
          return [d, { required: dep, missing: false }]
        } else {
          p = "missing: "
        }
        p += d + "@" + dep
